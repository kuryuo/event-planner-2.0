...
export const useNotificationsSignalR = (): void => {
  ...
  useEffect(() => {
    if (IS_MOCK_MODE || !accessToken) {
      return;
    }

    const connection = createHubConnection(
      resolveHubUrl(NOTIFICATIONS_HUB_URL),
      () => accessToken
    );
    let isCancelled = false;

    signalRDebug(
      "Init notifications hub",
      resolveHubUrl(NOTIFICATIONS_HUB_URL)
    );
    ...
    const handleIncoming = (payload: unknown) => {
      ...
      const looksLikeChat =
        type === "chatmessage" || Boolean(messageText && senderName);
      ...
      if (looksLikeChat && eventId) {
        ...
        signalRDebug(
          "Notification looks like chat but no eventId/community_id"
        );
      }
    ...
    connection.onreconnected(() => {
      signalRDebug(
        "Notifications hub reconnected -> invalidating Notification tag"
      );
      invalidateNotifications();
    });
    ...
    void (async () => {
      ...
      if (!started || isCancelled) {
        signalRDebug("Notifications hub not started or disposed", {
          started,
          isCancelled,
        });
        return;
