...
export const useChatNotificationsSignalR = (events: EventMeta[]): void => {
  ...
  useEffect(() => {
    if (IS_MOCK_MODE || !accessToken || events.length === 0) {
      return;
    }

    const connection = createHubConnection(
      resolveHubUrl(CHAT_HUB_URL),
      () => accessToken
    );
    ...
    const joinAllEvents = async () => {
      for (const event of events) {
        try {
          ...
          signalRDebug("Joined chat event", {
            eventId: event.id,
            eventName: event.name,
          });
        } catch {
          signalRDebug("Failed to join chat event", {
            eventId: event.id,
            eventName: event.name,
          });
        }
    ...
    const onMessageReceived = (payload: unknown) => {
      ...
      if (!eventId) {
        signalRDebug(
          "Skip chat notification: no eventId/community_id in payload"
        );
        return;
      }
